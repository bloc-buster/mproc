#include <errno.h>
#include <stdlib.h>
#include <stdio.h>
#include <sys/wait.h>
#include <dirent.h>
#include <string.h>
#include <fcntl.h>
#include <ctype.h>
#include <signal.h>
#include <time.h>
#include <pthread.h>
#include <sys/time.h>
#include <sys/types.h>
#include <sys/stat.h>
#include <sys/param.h>
#include <unistd.h>
#include <iostream>
#include <cmath>
using namespace std;

void instructions(){
		printf("\nusage\n");
		printf("\nprepare files: ./blocbuster input.txt output.gml threshold numInd numSNPs numHeaderRows numHeaderCols granularity1 (default 1) granularity2 (default 7) max_simultaneous_processes (default 15) output_folder (default log_files) semaphores (0 or 1 default 0)\n");
		printf("\n(wait until jobs complete)\n");
		printf("\ncombine files: ./blocbuster -z\n\n");
		exit(1);
}

int series(int n){
	float N = ((float)n * (float)(n+ 1)) / 2.0;
	return (int)N;
}

int main(int argc,char ** argv){
	/*
	fprintf(stdout,"command line args %d\n",argc);
	for(int i = 0; i < argc; ++i){
		fprintf(stdout,"%s ",argv[i]);
	}
	fprintf(stdout,"\n");
	printf("file name %s\n", __FILE__);
	*/
	char dir[MAXPATHLEN+1];// working directory
	getcwd(dir,MAXPATHLEN+1);
	char * runfile = realpath(argv[0],NULL);
	//fprintf(stdout,"working directory %s runfile %s\n",dir,runfile);
	char runfile1[100];
	bool found = false;
        for (int i = strlen(runfile) - 1; i >= 0; i--) {
           if (runfile[i] == '/') {
	      found = true;
              strncpy(runfile1,runfile,i+1);
	      runfile1[i+1] = '\0';
              break;
           }
        }
	bool computeN = false;
	int options = 0;
	int g1 = 1;
	int g2 = 7;
	int procs = 15;
	char * log_folder = (char *)"log_files";
	int semaphores = 0;
	int ch;
	while((ch=getopt(argc,argv,"g:G:hno:p:s:z")) != -1){
		if(ch=='g'){
			g1 = atoi(optarg);
			++options;
			++options;
		} else if(ch=='G'){
			g2 = atoi(optarg);
			++options;
			++options;
		} else if(ch=='h'){
			instructions();
		} else if(ch=='n'){
			computeN = true;
			++options;
		} else if(ch=='o'){
			log_folder = optarg;
			++options;
			++options;
		} else if(ch=='p'){
			procs = atoi(optarg);
			++options;
			++options;
		} else if(ch=='s'){
			semaphores = atoi(optarg);
			++options;
			++options;
		} else if(ch=='z'){
			char paramfile1[100];
			bool found = false;
                        for (int i = strlen(runfile) - 1; i >= 0; i--) {
                           if (runfile[i] == '/') {
			      found = true;
		              strncpy(paramfile1,runfile,i+1);
			      paramfile1[i+1] = '\0';
                              break;
                           }
	                }
			//fprintf(stdout,"%s\n",paramfile1);
			char paramfile2[150];
			char mainfile[150];
			char cccfile[150];
			if(found==true){
			   sprintf(paramfile2,"%sparams.h\0",paramfile1);
			   sprintf(mainfile,"%smain.sh\0",paramfile1);
			   sprintf(cccfile,"%sccc.sh\0",paramfile1);
			} else {
			   sprintf(paramfile2,"params.h\0");
			   sprintf(mainfile,"main.sh\0");
			   sprintf(cccfile,"ccc.sh\0");
			}
			FILE * file = fopen(paramfile2,"r");
			if(file==NULL){
				printf("error - could not read params file\n");
				exit(1);
			}
			char buffer[200];
			char * token;
			char * gmlfile;
			char * tempfolder;
			char * granularity2;
			char * prevaddress;
			while(fgets(buffer,1000,file) != NULL){
				token = strtok(buffer," ");
				while(token != NULL){
					if(strcmp(token,"gml_file")==0){
						token = strtok(NULL," ");
						gmlfile = strdup(token);
						break;
					} else if(strcmp(token,"temp_folder")==0){
						token = strtok(NULL," ");
						tempfolder = strdup(token);
						break;
					} else if(strcmp(token,"granularity2")==0){
						token = strtok(NULL," ");
						granularity2 = strdup(token);
						break;
					} else if(strcmp(token,"runfile")==0){
						token = strtok(NULL," ");
						prevaddress = strdup(token);
						break;
					}
					token = strtok(NULL," ");
				}
			}
			fclose(file);
			prevaddress[strlen(prevaddress) - 1] = '\0';//remove newline
			char command[150];
			if(atoi(granularity2) > 0){
				sprintf(command,"srun %smain.sh %s", prevaddress, strtok(tempfolder,"\n"));
			} else {
				char * temp = strtok(tempfolder,"\n");
				temp[strlen(temp) - 1] = '\0';
				sprintf(command,"sbatch --output=%s%%j.out\" %s %s\" %s", temp, cccfile,temp, strtok(gmlfile,"\n"));
			}
			//fprintf(stdout,"running command\n");
			//fprintf(stdout,"%s\n",command);
			system(command);
			//FILE* f = popen("echo $?", "r");
			//int errcode;
			//fscanf(f,"%d",&errcode);
			//pclose(f);
			//printf("returning code %d\n",errcode);
			return 0;
		}
	}
	if(argc - options < 8 || argc - options > 13){
		instructions();
	}
	int argindex = 1 + options;
	//printf("arg index %d\n",argindex);
	char * inputfile = realpath(argv[argindex++], NULL);
	//printf("input file %s\n",inputfile);
	FILE * fptr = fopen(inputfile,"r");
	if(fptr==NULL){
		printf("error - could not open input file %s\n",argv[argindex - 1]);
		exit(1);
	}
	fclose(fptr);

  	char filestem[100];// path to input data file
        for (int i = strlen(inputfile) - 1; i >= 0; i--) {
             if (inputfile[i] == '/') {
		strncpy(filestem,inputfile,i+1);
		filestem[i+1] = '\0';
                break;
             }
	}
	if(strlen(filestem)==0){ // did not find /
		fprintf(stderr,"error - please place input file in a different folder\n");
		exit(1);
	}
	char * output = argv[argindex++];
	//printf("%s\n",output);
	char * substr = strstr(output,".gml");
	if(substr==NULL || strcmp(substr,".gml") != 0){
		fprintf(stderr,"error - output file must end with .gml, %s\n",output);
		exit(1);
	}
	if(strstr(output,"/") != NULL){
		fprintf(stderr,"error - outputfile must be a file name, not a file path\n");
		fprintf(stderr,"%s\n",output);
		exit(1);
	}
	char outputfile[strlen(filestem) + strlen(output) + 1];
	sprintf(outputfile,"%s%s\0",filestem,output);
	//printf("%s\n",outputfile);
	//char outputfile[100];
	//sprintf(outputfile,"%s%s.gml\0",filestem,__FILE__);
	//printf("filestem %s output %s substring %s outputfile %s\n",filestem,output,substr,outputfile);

	float thresh = atof(argv[argindex++]);
	if(thresh <= 0 || thresh > 1){
		printf("error - threshold must be between 0 and 1\n");
		exit(1);
	}
	int numind = atoi(argv[argindex++]);
	if(numind < 1){
		printf("error - samples < 1\n");
		exit(1);
	}
	int numsnps = atoi(argv[argindex++]);
	if(numsnps < 1){
		printf("error - snps < 1\n");
		exit(1);
	}
	int headerrows = atoi(argv[argindex++]);
	if(headerrows < 0){
		printf("error - header rows < 0\n");
		exit(1);
	}
	int headercolumns = atoi(argv[argindex++]);
	if(headercolumns < 0){
		printf("error - header columns < 0\n");
		exit(1);
	}
	//printf("%d\n",headercolumns);
	if(argc - options >= 9){
		g1 = atoi(argv[argindex++]);
		if(g1 < 1){
			printf("error - granularity < 1\n");
			exit(1);
		} else if(g1 > numsnps){
			printf("error - granularity greater than snps\n");
			exit(1);
		}
	}
	//printf("%d\n",g1);
	if(argc - options >= 10){
		g2 = atoi(argv[argindex++]);
		if(g2 < 0){
			printf("error - granularity < 0\n");
			exit(1);
		} else if(g2 > numsnps / g1){
			printf("error - granularity 2 greater than snps divided by granularity 1\n");
			exit(1);
		}
		/*
		if(g1 > 1 && g2 > 7){
			printf("error - granularities larger than 7 may result in excessive processes generated\n");
			exit(1);
		}
		if(g1 > 7 && g2 > 0){
			printf("error - first granularity greater than 7 not recommended\n");
			exit(1);
		}
		*/
	}
	//printf("%d\n",g2);
	if(computeN==true){
		int N = series(g1);
		if(g2==0){
			printf("jobs %d\n",N);
		} else {
			int N2 = (g1 * series(g2)) + (series(g1 - 1) * pow(g2,2));			
			printf("processes %d\n", N2);
			//float N2 = ((float)g2 * (float)(g2 + 1)) / 2.0;
			//float totalprocesses = N2 * N;
			//printf("%d jobs * %d processes = %d total processes\n",(int)N,(int)N2,(int)totalprocesses);
		}
		exit(0);
	}
	//printf("did not compute n\n");
	if(argc - options >= 11){
		procs = atoi(argv[argindex++]);
		if(procs < 1){
			printf("error - max processes < 1\n");
			exit(1);
		}
	}
	//printf("%d\n",procs);
	if(argc - options >= 12){
		log_folder = argv[argindex++];
		if(strstr(log_folder,"/") != NULL){
			fprintf(stderr,"error - temp output folder must be a folder name, not a folder path\n");
			exit(1);
		}
	}
	char outputfolder[strlen(filestem) + strlen(log_folder) + 1];
	sprintf(outputfolder,"%s%s/\0",filestem,log_folder);
	if(argc - options >= 13){
		semaphores = atoi(argv[argindex++]);
		if(semaphores < 0 || semaphores > 1){
			printf("error - semaphores must be equal to 0 or 1\n");
			exit(1);
		}
	}
	//printf("%s\n",outputfolder);
	char command[150];
	sprintf(command,"srun %smain.sh %s %s %f %d %d %d %d %d %d %d %s %d",runfile1,inputfile,outputfile,thresh,numind,numsnps,headerrows,headercolumns,g1,g2,procs,outputfolder,semaphores);
	//fprintf(stdout,"blocbuster.cpp running command %s\n",command);
	system(command);
	return 0;
}

